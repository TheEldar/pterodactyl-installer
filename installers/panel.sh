...
letsencrypt() {
  ...
  # Obtain certificate
  certbot --nginx --redirect --no-eff-email --email "$email" -d "$FQDN" --server https://eldar-ca.intern.cyber-network.de:8443/acme/acme/directory --agree-tos --key-type 'RSA' --cert-name "$FQDN" || FAILED=true
